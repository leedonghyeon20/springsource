...
public class NovelService {
    ...
    public PageResultDTO<NovelDTO> getList(PageRequestDTO pageRequestDTO) {
        ...
        List<NovelDTO> dtoList = result.get().map(arr -> {
            ...
            NovelDTO novelDTO = NovelDTO.builder()
                    ...
                    .rating(raiting != null ? raiting.intValue() : 0)
                    .build();
    ...
    public NovelDTO getRow(Long id) {
        ...
        Double raiting = (Double) result[2];

        NovelDTO dto = entityToDto(novel, genre, raiting);
        return dto;
    }

    // private Novel dtoToEntity(NovelDTO dto)

    private NovelDTO entityToDto(Novel novel, Genre genre, Double raiting) {
        NovelDTO novelDTO = NovelDTO.builder()
                .id(novel.getId())
                .title(novel.getTitle())
                .author(novel.getAuthor())
