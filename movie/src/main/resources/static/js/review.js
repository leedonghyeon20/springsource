...
const reviewList = () => {
  ...
  axios.get(`/reviews/${mno}/all`).then((res) => {
    ...
    data.forEach((review) => {
      ...
      result += `<div><span class="font-semibold">${review.text}</div>`;
      result += `<div class="text-muted"><span class="small"><span class="d-inline-block mr-3">${review.nickname}</span>`;
